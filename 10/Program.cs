// Гонка - из разных потоков в 1 ячейку записываем данные, что может вывести неверные значения
const int THREADS_NUMBER = 4; //число потоков
const int N = 100000; //размер массива
object lock_object = new object(); // для создания блока с LOCK

//сортировка подсчетом
//int[] array = {-10, -5, -9, 0, 2, 5, 1, 3, 1, 0, 1};
//int[] sortedArray = CountingSortExtended(array);
Random rand = new Random(); // Создание рандомных чисел упращёнка
int[] resSerial = new int[N].Select(r => rand.Next(0, 5)).ToArray(); // создаём рандомное число от 0 до 5 в переме. r и вставлвяем в массив new int
...
void PrepareParallelCountingSort(int[] inputArray)
{
    ...
    foreach(var thread in threadsParall)    // блок для ожидания потока, чтобы выполнился до конца
    {
        thread.Join();
    }
...
void CountingSortParallel(int[] inputArray, int[] counters, int offset, int startPos, int endPos)
{
    ...
    {
        lock (lock_object)  // вот и блокировка потока
        {
