...
int[,] threadMulRes = new int[N, N]; // параллельное умножение матриц

int[,] firstMatrix = MatrixGenerator(N, N);
int[,] secondMatrix = MatrixGenerator(N, N);

SerialMatrix(firstMatrix, secondMatrix);
...
void PrepareParallelMatrix(int[,] a, int[,] b)
{
    ...
    for (int i = 0; i < THREADS_NUMBER; i++)
    {
        ...
        threadList.Add(new Thread(() => ParallelMatrix(a, b, startPos, endPos)));
        threadList[i].Start();
    }
    for (int i = 0; i < THREADS_NUMBER; i++)
    {
...
void ParallelMatrix(int[,] a, int[,] b, int startPos, int endPos)
{
